...
// 1. SACAMOS LOS ICONOS FUERA PARA QUE NO SE RECREEN EN CADA RENDER (Cura el lag)
...
export function Step1Core({ wizard }: StepProps) {
  ...
  const dockerImagePrefix = selectedTech?.id === 'nodejs' ? 'node' : selectedTech?.id === 'java' ? 'openjdk' : selectedTech?.id
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
          {technologies.map((tech) => {
            ...
            return (
              <button key={tech.id} onClick={() => handleLanguageSelect(tech)}
                // 2. REDUCIMOS LA DURACIÓN A duration-75 PARA EVITAR COLAPSO DE LA GPU
                className={cn('relative flex flex-col items-center gap-2.5 rounded-lg border p-4 interactive-card cursor-pointer text-center transition-all duration-75',
                  isSelected ? `card-selected ${tech.bgClass} border-primary` : 'border-border bg-muted/30 hover:bg-muted/50')}
              >
      ...
      {selectedTech && (
        <div className="grid sm:grid-cols-2 gap-4 animate-slide-up">
          ...
          <div>
            ...
            <input type="text" value={state.startCommand} onChange={(e) => updateState({ startCommand: e.target.value })}
